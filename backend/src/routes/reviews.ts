...
router.get('/product/:productId', asyncHandler(async (req: Request, res: Response): Promise<void> => {
...
router.post('/product/:productId', authenticate, asyncHandler(async (req: Request, res: Response): Promise<void> => {
  const productId = parseInt(req.params.productId as string, 10);
  if (isNaN(productId)) {
    res.status(400).json({ error: 'ID produit invalide' });
    return;
